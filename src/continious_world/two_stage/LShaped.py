...
class FirstStage:
    """
    Represents the first stage of a two stage stochastic program.

    Attributes
    ----------

    A: numpy.matrix, scipy.sparse.
        The coefficients
...
class TwoStageSP:
    ...
    def multi_cut_l_shaped(self, reset=False):
                ...
                if feasibility:
                    ...
                    if round(theta_hat[k], 4) < round(wk, 4):
                        ...
                        self.first_stage.model.addConstr(
                            E @ self.first_stage.x + self.first_stage.theta[k] >= e, name=f'OC_{k}{s[k].count_}')
                        s[k].count()
                    ...
                    self.first_stage.model.addConstr(D @ self.first_stage.x >= d, name=f'FC_{k}{r.count_}')
        ...
        self.outputLog.info(f'\nMulti-cut L-shaped converged\n{"-" * 100}' + \
                         ...
                         f'\nTotal cuts:\t{r.count_ + sum(o.count_ for o in s)}' + \
                         f'\n\tfeasibility:\t{r.count_}({100 * round(r.count_ / (r.count_ + sum(o.count_ for o in s)), 3)}%)' + \
                         f'\n\toptimality:\t{sum(o.count_ for o in s)}' + \
                         f'({round(100 * sum(o.count_ for o in s) / (r.count_ + sum(o.count_ for o in s)), 3)}%)')
        return r, s, v
    ...
    def l_shaped(self, reset=False):
        ...
        while not stop:
            ...
            for sp in self.second_stage_sps:
                ...
                if feasibility:
                    ...
                    # Add feasibility cuts
                    self.first_stage.model.addConstr(D @ self.first_stage.x >= d, name=f'FC_{r.count_}')
                    self.outputLog.info(f'-inf\tinf\t-\tFC')
            ...
            if not infeasible:
                ...
                self.first_stage.model.addConstr(
                    E @ self.first_stage.x >= e - self.first_stage.theta, name=f'OC_{s.count_}')
        ...
        self.outputLog.info(f'\nL-shaped converged\n{"-" * 100}' + \
                         ...
                         f'\nTotal cuts:\t{r.count_ + s.count_}' + \
                         f'\n\tfeasibility:\t{r.count_}({100 * round(r.count_ / (r.count_ + s.count_), 3)}%)' + \
                         f'\n\toptimality:\t{s.count_}' + \
                         f'({round(100 * s.count_ / (r.count_ + s.count_), 3)}%)')
        return r, s, v
    ...
    def optimality_gap(self, upper_bound_sample, lower_bound_samples, alpha=0.01):
        ...
        self.outputLog.info(f'An estimation of the optimality gap with a confidence of {100 * (1 - alpha)}%' + \
                            f' is {ub - lb}, i.e., a percentage gap of {(ub - lb) / ub}%')
        return ub - lb
